"""
Upbit WebSocket 模組
用於連接 Upbit WebSocket API 並接收加密貨幣即時價格
"""
import json
import threading
import time
...
class UpbitWebSocket:
    def __init__(self, callback: Callable[[str, float, str], None]):
        ...
        """
        self.base_url = "wss://api.upbit.com/websocket/v1"
    ...
    def start(self):
        ...
        logger.info("Upbit WebSocket 已啟動")
        
    def stop(self):
        """停止 WebSocket 連接"""
        self.is_running = False
        self._disconnect()
        logger.info("Upbit WebSocket 已停止")
    ...
    def subscribe(self, symbol: str):
        ...
        if symbol not in self.subscribed_symbols:
            ...
            try:
                ...
                logger.debug("Upbit ws 尚未就緒，訂閱將在連線建立後送出")
            logger.info(f"已登錄 Upbit {symbol} 訂閱（將在連線可用時發送）")
    ...
    def unsubscribe(self, symbol: str):
        ...
        if symbol in self.subscribed_symbols:
            ...
            try:
                ...
                logger.info(f"已取消 Upbit {symbol} 訂閱（如連線可用已發送取消請求）")
            except Exception:
                logger.debug("Upbit ws 尚未就緒，取消訂閱已從本地清單移除")
    ...
    def _run(self):
        ...
        while self.is_running:
            try:
                ...
                logger.info(f"正在連接 Upbit WebSocket: {self.base_url}")
                ...
                logger.error(f"Upbit WebSocket 錯誤: {e}")
                if self.is_running:
                    time.sleep(self.reconnect_delay)
                    
    def _on_open(self, ws):
        """WebSocket 連接建立時的callback"""
        logger.info("Upbit WebSocket 連接已建立")
        # 連線建立後，向 Upbit 發送目前已登記的所有訂閱
        try:
            for symbol in list(self.subscribed_symbols):
                try:
                    self._subscribe(ws, symbol)
                except Exception:
                    logger.exception(f"向 Upbit 發送訂閱 {symbol} 時發生錯誤")
        except Exception:
        ...
        if not self.ws_ping_thread or not self.ws_ping_thread.is_alive():
            ...
            logger.info("已啟動 Upbit WebSocket 定期 ping 執行緒")
            ...
            if 'instId' in data and 'px' in data:
                ...
                if symbol_full.endswith('USDT'):
                    ...
                    if symbol not in self.last_prices or self.last_prices[symbol] != price:
                        ...
                        self.callback(symbol, price, 'Upbit')
                        logger.debug(f"Upbit {symbol} 價格更新: ${price:,.2f}")
    ...
    def _subscribe(self, ws, symbol: str):
        ...
        {
            "type": "trade",
            "codes": [f"KRW-{symbol}"], # Upbit 的 USDT 交易量極低
            "isOnlyRealtime": True
        },
        ]
        if not ws:
        ...
        try:
            ...
            logger.info(f"已向 Upbit WebSocket 發送訂閱請求: {symbol}USDT")
        except Exception:
            logger.exception(f"發送訂閱請求 {symbol}USDT 時發生錯誤")

    def _unsubscribe(self, ws, symbol: str):
        """向 Upbit WebSocket 發送取消訂閱請求"""
        if not self.is_running:
            return 
        logger.info(f"Upbit WebSocket 不支援取消訂閱，將在本地移除 {symbol} 訂閱後重新連線")
            
    def _on_error(self, ws, error):
        """WebSocket 錯誤時的callback"""
        logger.error(f"Upbit WebSocket 錯誤: {error}")
    ...
    def _on_close(self, ws, close_status_code, close_msg):
        ...
        logger.info(f"Upbit WebSocket 已關閉 (代碼: {close_status_code}, 訊息: {close_msg})")
    ...
    def _initiate_ping(self):
        ...
        while self.is_running and self.ws:
            try:
                self.ws.send("PING")
                logger.debug("已向 Upbit WebSocket 發送 ping")
            except Exception as e:
...
if __name__ == "__main__":
    ...
    # 建立 WebSocket 實例
    Upbit_ws = UpbitWebSocket(callback=test_callback)
    
    # 啟動
    Upbit_ws.start()

    # 訂閱幣種
    Upbit_ws.subscribe("BTC")
    Upbit_ws.subscribe("ETH")
